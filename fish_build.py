...
# ── Card CSS/JS (no inner scrollbars; debounced height; tooltips) ─────────────
...
<style>
  ...
  @media (prefers-color-scheme: dark){
    :root{ --astro-bg:#0b1020; --astro-fg:#e5e7eb; --astro-sub:#9aa4b2; --astro-border:#1f2937; --astro-shadow: 0 8px 30px rgba(0,0,0,.45); }
  }
  .wrap{font-family:var(--astro-font); background:transparent;}
  .card{ width:100%; max-width:none; border:1px solid var(--astro-border); border-radius:var(--astro-radius);
  ...
  th, td{ padding:10px; border-top:1px solid var(--astro-border); text-align:left; font-size:14px;
          overflow:visible; white-space:nowrap; vertical-align:middle; text-overflow:ellipsis; }
  ...
  .info-dot{display:inline-block; width:1.35em; height:1.35em; line-height:1.35em; border-radius:999px;
            background:var(--astro-sub); color:#fff; font-weight:700; font-size:12px; text-align:center}
  .tip-bubble{ display:none; position:absolute; left:50%; transform:translateX(-50%); bottom:calc(100% + 8px);
...
(function(){{
  ...
  function postH() {{
    ...
    if (h !== lastH) {{
      lastH = h;
      try {{ parent.postMessage({{type:"{message_type}", height:h}}, "*"); }} catch(e) {{}}
    }}
  }}

  function schedule() {{
    if (rafId) cancelAnimationFrame(rafId);
    rafId = requestAnimationFrame(postH);
  }}

  window.addEventListener("load", schedule);
  window.addEventListener("resize", schedule);
  if ("ResizeObserver" in window) new ResizeObserver(schedule).observe(document.body);
  setTimeout(schedule, 200); // one settle try
}})();
</script>
"""
...
def to_utc_naive(dt_local):
    """Meteosource local-aware -> UTC-naive"""
    try: return dt_local.astimezone(timezone.utc).replace(tzinfo=None)
    except: return dt_local
...
def score_tide(dt_utc: datetime, heights: List[Dict], extremes: List[Dict]) -> Tuple[float,str]:
    ...
    def hours_to(ex): return abs((ex["dt"] - dt_utc).total_seconds())/3600.0 if ex else None
    h_next = hours_to(next_ext)
    next_lab = f"next {next_ext['type'].lower()} in {h_next:.1f}h" if next_ext and h_next is not None else "next:?"

    s_move = min(100.0, 40.0 + min(60.0, abs(rate)*300.0))
    s_phase = 5.0 if rate>0.03 else 0.0
    s_slack_pen = -15.0 if phase=="slack" else 0.0
    timing_bonus = 6.0 if next_ext and next_ext["type"]=="High" and h_next<=2.0 else (10.0 if next_ext and next_ext["type"]!="High" and h_next<=2.0 else 0.0)

    score = max(0.0, min(100.0, s_move + s_phase + s_slack_pen + timing_bonus))
    note = f"tide={phase} {rate:+.2f} m/h, {next_lab}"
    return score, note

WEIGHTS = dict(wind=0.25, clouds=0.08, precip=0.08, pressure=0.08, humidity=0.05, wave=0.20, seatemp=0.06, tide=0.20)

# ── WorldTides fetch (UTC-naive) ──────────────────────────────────────────────
def wt_request(params: dict) -> Optional[dict]:
    url = "https://www.worldtides.info/api"
    try:
        r = requests.get(url, params=params, timeout=20)
        if r.status_code != 200:
            print(f"[WorldTides] HTTP {r.status_code}: {r.text[:160]}")
            return None
        return r.json()
...
def fetch_worldtides(lat: float, lon: float, start_utc: datetime, days: int, step_s: int=3600, key: Optional[str]=None):
    if not key: return dict(heights=[], extremes=[])
    start_ts = int(time.mktime(start_utc.timetuple()))
    length_s = int(days * 86400)

    j_ext = wt_request(dict(extremes="", lat=lat, lon=lon, start=start_ts, length=length_s, key=key)) or {}
    extremes = [dict(dt=datetime.utcfromtimestamp(int(ex.get("dt") or ex.get("time"))), type=ex.get("type",""), height=float(ex.get("height",0.0)))
                for ex in (j_ext.get("extremes") or []) if (ex.get("dt") or ex.get("time")) is not None]

    j_h = wt_request(dict(heights="", lat=lat, lon=lon, start=start_ts, length=length_s, step=step_s, key=key)) or {}
    heights = [dict(dt=datetime.utcfromtimestamp(int(h.get("dt") or h.get("time"))), height=float(h.get("height",0.0)))
               for h in (j_h.get("heights") or []) if (h.get("dt") or h.get("time")) is not None]

    heights.sort(key=lambda x: x["dt"])
    extremes.sort(key=lambda x: x["dt"])
    return dict(heights=heights, extremes=extremes)

# ── Build payload (group by day: one best 2h window + tides) ──────────────────
def build_payload():
    ...
    heights, extremes = wt.get("heights", []), wt.get("extremes", [])

    # Build local-date -> list of local-time extremes for display
    extremes_by_day: Dict[str, List[Dict]] = {}
    for ex in extremes:
        loc = ex["dt"].replace(tzinfo=timezone.utc).astimezone(ZLOCAL)
        key = loc.date().isoformat()
        extremes_by_day.setdefault(key, []).append({"type": ex["type"], "local": loc})
    for k in extremes_by_day:
        extremes_by_day[k].sort(key=lambda e: e["local"])
    ...
    for day, hrs in sorted(by_day.items()):
        ...
        for i in range(len(hrs)-1):
            seg = hrs[i:i+2]
            s = mean([w["score"] for w in seg])
            t0, t1 = seg[0]["t"], seg[-1]["t"] + timedelta(minutes=59)
            note = "; ".join(w["notes"] for w in seg)
...
def render_card(payload: dict) -> str:
    ...
    if not wins:
        rows = "<tr><td colspan='7' class='dim'>No data</td></tr>"
    else:
        rows = ""
        for w in wins:
            tip_html = (
